...
if [[ -n "${C311_ARTIFACT_DIR:-}" ]]; then
  ...
  artifact_dir="$(mktemp -d "${TMPDIR:-/tmp}/c311-fe07.XXXXXX")"
fi
...
package_artifact_dir="$(mktemp -d "${TMPDIR:-/tmp}/c311-fe07-packages.XXXXXX")"
cleanup () {
...
corepack yarn --cwd "$repo_root/client/web/compose" serve --port "$compose_port" >"$artifact_dir/compose-fe07.log" 2>&1 & compose_pid=$!
corepack yarn --cwd "$repo_root/client/web/admin" serve --port "$admin_port" >"$artifact_dir/admin-fe07.log" 2>&1 & admin_pid=$!
...
C311_ADMIN_URL="http://127.0.0.1:$admin_port" C311_ARTIFACT_DIR="$artifact_dir" PYTHONPATH="$repo_root/tools/c311-browser" python3 "$repo_root/tools/c311-browser/fe07_matrix.py"
