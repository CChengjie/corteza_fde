...
func TestContextualHelpAdministrationContractIsComplete(t *testing.T) {
	...
	platformCapabilities := fixtures["platform_administrator"].([]string)
	for _, name := range helpOperations {
		if !contains(platformCapabilities, name) {
			t.Errorf("platform administrator fixture must grant contextual-help operation %s", name)
		}
	}
	for _, name := range []string{
