...
func TestDraftLifecycleUsesConstituentOwnershipAndOptimisticConcurrency(t *testing.T) {
	...
	links, _, err := store.SearchCity311RequestConstituentLinks(ctx, st, composeTypes.City311RequestConstituentFilter{RequestID: draftID(draft)})
	require.NoError(t, err)
	require.Len(t, links, 1)
	require.Equal(t, contract.RelationshipPrimaryRequester, links[0].RelationshipType)
	require.Equal(t, "C-"+strconv.FormatUint(user.ID, 10), links[0].ConstituentID)
	require.True(t, links[0].PortalVisible)
	...
	portalList, err := svc.ListPortalRequests(ctx, user.ID, PortalRequestFilter{})
	require.NoError(t, err)
	require.Len(t, portalList.Items, 1)
	require.Equal(t, stored.RequestNumber, portalList.Items[0].RequestNumber)
