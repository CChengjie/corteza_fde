...
export class C311HttpProvider implements C311Provider {
  ...
  listAdminHelpVersions (helpKey: HelpKey, query: ListQuery & { language?: Language } = {}): Promise<PageResponse<HelpContent>> { const { language, ...paging } = query; return this.request<PageResponse<HelpContent>>({ method: 'GET', path: `/api/v1/admin/help/${encodeURIComponent(helpKey)}/versions`, query: { ...this.listQuery(paging), ...(language ? { language } : {}) } }).then(value => this.markSanitizedContentPage(value)) }
