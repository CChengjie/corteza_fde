<template>
  <c311-app-shell title="City 311 operations" main-id="c311-operations-main">
    ...
    <main>
      ...
      <section class="mb-4" aria-labelledby="calendar-heading"><h2 id="calendar-heading" class="h4">Reminder calendar</h2><label for="calendar-ics">ICS calendar</label><textarea id="calendar-ics" v-model="calendarICS" class="form-control" rows="7" spellcheck="false" /><button class="btn btn-primary mt-2" :disabled="busy" @click="importCalendar">Import ICS</button><button class="btn btn-outline-primary mt-2 ml-2" :disabled="busy" @click="exportCalendar">Export ICS</button><a v-if="calendarDownload" :href="calendarDownload" download="city311-reminders.ics" class="btn btn-link mt-2">Download exported calendar</a></section>

      <section class="mb-4" aria-labelledby="workflow-heading"><h2 id="workflow-heading" class="h4">Workflows</h2><div class="table-responsive"><table class="table table-sm"><thead><tr><th>Name</th><th>Trigger</th><th>Active</th><th>Version</th><th /></tr></thead><tbody><tr v-for="workflow in workflows" :key="workflow.workflow_id"><td>{{ workflow.name }}</td><td>{{ workflow.trigger }}</td><td>{{ workflow.active ? 'Yes' : 'No' }}</td><td>{{ workflow.version }}</td><td><button class="btn btn-sm btn-link" type="button" @click="editWorkflow(workflow)">Edit</button><button class="btn btn-sm btn-link" type="button" @click="testWorkflow(workflow)">Test</button><button class="btn btn-sm btn-link" type="button" @click="toggleWorkflow(workflow)">{{ workflow.active ? 'Deactivate' : 'Activate' }}</button></td></tr></tbody></table></div><form @submit.prevent="saveWorkflow"><label for="workflow-definition">Workflow definition</label><textarea id="workflow-definition" v-model="workflowJSON" class="form-control font-monospace" rows="12" spellcheck="false" /><button class="btn btn-primary mt-2" :disabled="busy">{{ selectedWorkflow ? 'Update workflow' : 'Create workflow' }}</button><button v-if="selectedWorkflow" class="btn btn-link mt-2" type="button" @click="resetWorkflow">Cancel</button></form><h3 class="h6 mt-3">Execution log</h3><p v-if="!executions.length" class="text-muted" role="status">No workflow executions found.</p><ul v-else class="list-group"><li v-for="execution in executions" :key="execution.execution_id" class="list-group-item"><strong>{{ execution.workflow_id }}</strong> {{ execution.status }} <button class="btn btn-sm btn-link" type="button" @click="loadExecution(execution.execution_id)">Details</button></li></ul></section>

      <section class="mb-4" aria-labelledby="reports-heading"><h2 id="reports-heading" class="h4">Reports and exports</h2><div class="table-responsive"><table class="table table-sm"><thead><tr><th>Name</th><th>Entity</th><th>Version</th><th /></tr></thead><tbody><tr v-for="report in reports" :key="report.report_id"><td>{{ report.name }}</td><td>{{ report.entity }}</td><td>{{ report.version }}</td><td><button class="btn btn-sm btn-link" type="button" @click="editReport(report)">Edit</button><button class="btn btn-sm btn-link" type="button" @click="runReport(report)">Run</button><button class="btn btn-sm btn-link" type="button" @click="exportReport(report)">Export CSV</button></td></tr></tbody></table></div><form @submit.prevent="saveReport"><label for="report-definition">Report definition</label><textarea id="report-definition" v-model="reportJSON" class="form-control font-monospace" rows="10" spellcheck="false" /><button class="btn btn-primary mt-2" :disabled="busy">{{ selectedReport ? 'Update report' : 'Create report' }}</button><button v-if="selectedReport" class="btn btn-link mt-2" type="button" @click="resetReport">Cancel</button></form><div class="form-row mt-3"><div class="form-group col-md-4"><label for="data-export-entity">Data export entity</label><select id="data-export-entity" v-model="dataExportEntity" class="form-control"><option value="service-requests">Service requests</option><option value="constituents">Constituents</option><option value="follow-up-actions">Follow-up actions</option><option value="audit-events">Audit events</option></select></div><div class="col-md-3 d-flex align-items-end"><button class="btn btn-outline-primary mb-3" :disabled="busy" @click="exportData">Export data</button></div></div><button class="btn btn-outline-primary" :disabled="busy" @click="exportAudit">Export audit events</button><button class="btn btn-outline-primary ml-2" :disabled="busy" @click="exportContacts">Export eligible contact emails</button></section>

      <section class="mb-4" aria-labelledby="mail-heading"><h2 id="mail-heading" class="h4">Rich email</h2><form @submit.prevent="sendMail"><div class="form-row"><div class="form-group col-md-5"><label for="mail-recipient">Recipient</label><input id="mail-recipient" v-model.trim="mail.to" class="form-control" type="email" required></div><div class="form-group col-md-7"><label for="mail-subject">Subject</label><input id="mail-subject" v-model.trim="mail.subject" class="form-control" required></div></div><label for="mail-html">HTML body</label><textarea id="mail-html" v-model="mail.html" class="form-control" rows="6" /><button class="btn btn-outline-primary mt-2" type="button" :disabled="busy" @click="previewMail">Preview</button><button class="btn btn-primary mt-2 ml-2" :disabled="busy">Send email</button></form></section>
...
export default {
  ...
  data: () => ({ busy: false, message: '', formErrors: [], result: '', workflows: [], executions: [], reports: [], workflowJSON: emptyWorkflow(), reportJSON: emptyReport(), selectedWorkflow: null, selectedReport: null, calendarICS: '', calendarDownload: '', dataExportEntity: 'service-requests', mail: { to: '', subject: '', html: '' } }),
  ...
  methods: {
    ...
    async load () { await this.run(async () => { const [workflows, executions, reports] = await Promise.all([this.provider.listWorkflows({ page_size: 100 }), this.provider.listWorkflowExecutions({ page_size: 100 }), this.provider.listReports({ page_size: 100 })]); this.workflows = workflows.items || []; this.executions = executions.items || []; this.reports = reports.items || [] }, 'Operations loaded.') },
    ...
    testWorkflow (workflow) { return this.run(() => this.provider.testWorkflow(workflow.workflow_id, { request_id: '' }), 'Workflow test accepted.') },
    ...
    exportData () { return this.run(() => this.provider.exportData(this.dataExportEntity, { page_size: 100 }), 'Data export completed.') },
    previewMail () { return this.run(() => this.provider.previewMail({ to: [this.mail.to], subject: this.mail.subject, text: this.mail.html.replace(/<[^>]*>/g, ''), html: this.mail.html }), 'Mail preview generated.') },
    sendMail () { return this.run(() => this.provider.sendMail({ to: [this.mail.to], subject: this.mail.subject, text: this.mail.html.replace(/<[^>]*>/g, ''), html: this.mail.html }), 'Mail delivery accepted.') },
  },
}
</script>
