...
export default {
  ...
  data: () => ({ state: 'loading', error: null, busy: false, message: '', formErrors: [], branding: null, content: null, selectedContentKey: '', selectedContent: null, categories: null, customFields: null, workflows: null, executions: null, category: { code: '', label: '' } }),
  ...
  methods: {
    ...
    async load () { this.state = 'loading'; this.error = null; const results = await Promise.allSettled([this.provider.getAdminBranding(), this.provider.listAdminContent({ page_size: 100 }), this.provider.listAdminCategories({ page_size: 100 }), this.provider.listAdminCustomFields({ page_size: 100 }), this.provider.listWorkflows({ page_size: 100 }), this.provider.listWorkflowExecutions({ page_size: 100 })]); const value = (index, fallback) => results[index].status === 'fulfilled' ? results[index].value : fallback; this.branding = value(0, null); const content = value(1, null); this.content = content ? content.items || [] : null; const categories = value(2, null); this.categories = categories ? categories.items || [] : null; const customFields = value(3, null); this.customFields = customFields ? customFields.items || [] : null; const workflows = value(4, null); this.workflows = workflows ? workflows.items || [] : null; const executions = value(5, null); this.executions = executions ? executions.items || [] : null; this.selectedContentKey = this.content?.[0]?.content_key || ''; this.selectContent(); if (results.every(result => result.status === 'rejected')) { this.error = results[0].reason; this.state = stateForError?.(this.error) || 'terminal-error' } else this.state = 'populated' },
