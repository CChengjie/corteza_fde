  ...
  <c311-app-shell :title="t('Staff requests')" main-id="c311-staff-main">
    ...
    <template v-else>
      ...
      <section v-if="detail" class="mt-4" aria-labelledby="c311-request-heading">
        ...
        <section class="mb-3" aria-labelledby="c311-relationships-heading"><h3 id="c311-relationships-heading" class="h5">Constituent relationships</h3><ul v-if="detail.constituent_links?.length" class="list-group"><li v-for="link in detail.constituent_links" :key="`${link.constituent_id}-${link.relationship_type}`" class="list-group-item">{{ link.constituent_id }} - {{ link.relationship_type }} <button class="btn btn-sm btn-link" type="button" :disabled="busy" @click="unlinkConstituent(link)">Remove</button></li></ul><p v-else class="text-muted">No linked constituents.</p></section>
        <section class="mb-3" aria-labelledby="c311-attachments-heading"><h3 id="c311-attachments-heading" class="h5">Attachments</h3><ul v-if="detail.attachments?.length" class="list-group"><li v-for="attachment in detail.attachments" :key="attachment.attachment_id" class="list-group-item"><button class="btn btn-link p-0" type="button" @click="downloadAttachment(attachment)">{{ attachment.filename }}</button> {{ attachment.media_type }}</li></ul><p v-else class="text-muted">No attachments.</p></section>
...
export default {
  ...
  methods: {
    ...
    reminderAction (reminder, action) { const input = action === 'SNOOZE' && this.reminderSnoozeAt ? { due_at: new Date(this.reminderSnoozeAt).toISOString() } : {}; return this.run(() => this.provider.actionStaffReminder(reminder.reminder_id, action, input), `Reminder ${action.toLowerCase()}d.`) },
