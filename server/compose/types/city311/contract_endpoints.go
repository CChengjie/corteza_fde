...
func addContractEndpoints(document *ContractDocument) {
	for _, spec := range []endpointSpec{
		...
		{"identity_configuration_update", "PATCH", "/api/v1/admin/identity", "platform_administrator", "identity_configuration", "identity_configuration", 200},
...
func addConsumedEndpoints(document *ContractDocument) {
	document.Endpoints["mapping_geocode"] = EndpointContract{
		...
		Authentication: "MAP_API_TOKEN supplied only to server runtime", RequiredHeaders: []string{"Authorization", "Content-Type"},
	...
	document.Endpoints["civicworks_work_order_create"] = EndpointContract{
		...
		Authentication:  "CIVICWORKS_API_TOKEN supplied only to server runtime",
	...
	document.Endpoints["civicworks_event_callback"] = EndpointContract{
		...
		Authentication:  "HMAC-SHA256 signature over exact request body",
...
func configureEndpointMechanics(document *ContractDocument) {
	...
	for _, name := range []string{"portal_service_request_submit", "mail_send", "workflow_action_execute", "civicworks_work_order_create"} {
		endpoint := document.Endpoints[name]
