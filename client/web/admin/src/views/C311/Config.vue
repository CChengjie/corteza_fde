    ...
    <div id="c311-admin-config" data-c311-admin-config>
      ...
      <section v-else-if="tab === 'branding' && branding" data-c311-branding>
        ...
        <form @submit.prevent="saveBranding()">
          <label for="c311-branding-organisation-name">Organisation name</label><input id="c311-branding-organisation-name" v-model.trim="brandingForm.organisation_name" class="form-control" data-c311-field="organisation_name">
          <label for="c311-branding-primary-colour">Primary colour</label><input id="c311-branding-primary-colour" v-model="brandingForm.primary_colour" class="form-control" type="color">
          <label for="c311-branding-accent-colour">Accent colour</label><input id="c311-branding-accent-colour" v-model="brandingForm.accent_colour" class="form-control" type="color">
          <label for="c311-branding-font-family">Font family</label><input id="c311-branding-font-family" v-model.trim="brandingForm.font_family" class="form-control">
          <label for="c311-branding-logo-url">Logo URL</label><input id="c311-branding-logo-url" v-model.trim="brandingForm.logo_url" class="form-control" type="url">
          <label for="c311-branding-public-header">Public header</label><input id="c311-branding-public-header" v-model="brandingForm.public_header" class="form-control">
          <label for="c311-branding-public-footer">Public footer</label><input id="c311-branding-public-footer" v-model="brandingForm.public_footer" class="form-control">
          <div class="mt-2"><button v-if="can('admin_branding_update')" type="submit" class="btn btn-primary mr-2" data-c311-action="save-branding">Save draft</button><button v-if="can('admin_branding_preview')" type="button" class="btn btn-outline-secondary mr-2" data-c311-action="preview-branding" @click="previewBranding">Preview</button><button v-if="can('admin_branding_publish')" type="button" class="btn btn-success" data-c311-action="publish-branding" @click="publishBranding">Publish</button></div>
        </form>
      ...
      <section v-else-if="tab === 'content'" data-c311-content>
        ...
        <template v-if="contentMode === 'content'">
          <label for="c311-content-key">Content key</label><select id="c311-content-key" v-model="contentKey" class="form-control" data-c311-field="content_key" @change="loadContent"><option v-for="key in contentKeys" :key="key" :value="key">{{ key }}</option></select>
          <label for="c311-content-body" class="mt-2">Content body</label><textarea v-if="content" id="c311-content-body" v-model="contentForm.body" class="form-control" rows="8" data-c311-field="body"></textarea>
          <div class="mt-2"><button v-if="can('admin_content_update')" type="button" class="btn btn-primary mr-2" data-c311-action="save-content" @click="saveContent()">Save draft</button><button v-if="can('admin_content_preview')" type="button" class="btn btn-outline-secondary mr-2" data-c311-action="preview-content" @click="previewContent">Preview</button><button v-if="can('admin_content_publish')" type="button" class="btn btn-success" data-c311-action="publish-content" @click="publishContent">Publish</button></div>
        ...
        <template v-else>
          <label for="c311-help-key">Help key</label><select id="c311-help-key" v-model="helpKey" class="form-control" data-c311-field="help_key" @change="loadHelp"><option v-for="key in helpKeys" :key="key" :value="key">{{ key }}</option></select>
          <label for="c311-help-language">Language</label><select id="c311-help-language" v-model="helpLanguage" class="form-control" data-c311-field="help_language" @change="loadHelp"><option v-for="language in languages" :key="language" :value="language">{{ language }}</option></select>
          <label for="c311-help-body" class="mt-2">Help body</label><textarea v-if="help" id="c311-help-body" v-model="helpForm.body" class="form-control" rows="8" data-c311-field="help_body"></textarea>
      ...
      <section v-else-if="tab === 'categories'" data-c311-categories>
        ...
        <form v-for="category in categories" :key="category.code" class="border p-2 mb-2" :data-c311-category="category.code" @submit.prevent="saveCategory(category)">
          ...
          <label :for="`c311-category-${category.code}-en`" class="sr-only">English label</label><input :id="`c311-category-${category.code}-en`" v-model.trim="category.labels.EN" class="form-control" aria-label="English label">
          <label :for="`c311-category-${category.code}-es`" class="sr-only">Spanish label</label><input :id="`c311-category-${category.code}-es`" v-model.trim="category.labels.ES" class="form-control" aria-label="Spanish label">
          <label :for="`c311-category-${category.code}-vi`" class="sr-only">Vietnamese label</label><input :id="`c311-category-${category.code}-vi`" v-model.trim="category.labels.VI" class="form-control" aria-label="Vietnamese label">
          <label :for="`c311-category-${category.code}-active`">Active</label><input :id="`c311-category-${category.code}-active`" v-model="category.active" type="checkbox">
          <button v-if="can('admin_categories_update')" type="submit" class="btn btn-sm btn-primary ml-2" :data-c311-action="`save-category-${category.code}`">Save</button>
        </form>
        <form v-if="can('admin_categories_create')" @submit.prevent="createCategory"><label for="c311-new-category-code" class="sr-only">Code</label><input id="c311-new-category-code" v-model.trim="newCategory.code" required pattern="[A-Z0-9_-]+" placeholder="Code" class="form-control"><label for="c311-new-category-label" class="sr-only">English label</label><input id="c311-new-category-label" v-model.trim="newCategory.label" required placeholder="English label" class="form-control mt-1"><button type="submit" class="btn btn-primary mt-2" data-c311-action="create-category">Add category</button></form>
      </section>
      ...
      <section v-else data-c311-fields>
        ...
        <form v-if="can(editingField ? 'admin_custom_fields_update' : 'admin_custom_fields_create')" @submit.prevent="saveField()">
          <label for="c311-field-key" class="sr-only">Key</label><input id="c311-field-key" v-model.trim="fieldForm.key" required pattern="[a-z][a-z0-9_.-]*" placeholder="Key" class="form-control" :disabled="!!editingField" data-c311-field="field_key">
          <label for="c311-field-label" class="sr-only">English label</label><input id="c311-field-label" v-model.trim="fieldForm.label" required placeholder="English label" class="form-control mt-1" data-c311-field="field_label">
          <label for="c311-field-entity" class="sr-only">Entity</label><select id="c311-field-entity" v-model="fieldForm.entity" class="form-control mt-1"><option value="service_request">service_request</option><option value="constituent">constituent</option></select>
          <label for="c311-field-type" class="sr-only">Field type</label><select id="c311-field-type" v-model="fieldForm.field_type" class="form-control mt-1" data-c311-field="field_type"><option v-for="type in fieldTypes" :key="type">{{ type }}</option></select>
          <label for="c311-field-required">Required</label><input id="c311-field-required" v-model="fieldForm.required" type="checkbox"><label for="c311-field-active">Active</label><input id="c311-field-active" v-model="fieldForm.active" type="checkbox">
          <label for="c311-field-choices">Ordered choices</label><textarea id="c311-field-choices" v-model="fieldForm.choices" class="form-control" data-c311-field="field_choices"></textarea>
          <label for="c311-field-default">Default value JSON</label><textarea id="c311-field-default" v-model="fieldForm.defaultValue" class="form-control" data-c311-field="field_default"></textarea>
          <label for="c311-field-validation">Validation JSON</label><textarea id="c311-field-validation" v-model="fieldForm.validation" class="form-control" data-c311-field="field_validation"></textarea>
          <button type="submit" class="btn btn-primary" data-c311-action="save-field">{{ editingField ? 'Save field' : 'Add field' }}</button><button v-if="editingField" type="button" class="btn btn-link" @click="resetField">Cancel</button>
        </form>
      </section>
    </div>
  </c311-app-shell>
...
export default {
  ...
  methods: {
    ...
    async saveBranding (draft) { const value = draft || { ...this.brandingForm }; try { this.clearStatus(); this.branding = await this.$C311.provider.updateBranding(value, { expectedVersion: this.branding.version }); this.message = 'Branding draft saved.'; await this.loadBranding() } catch (error) { this.handleError(error, 'branding', value, next => this.saveBranding(next)) } },
    ...
    async saveContent (draft) { const value = draft || { ...this.contentForm }; try { this.clearStatus(); this.content = await this.$C311.provider.updateAdminContent(this.contentKey, value, { expectedVersion: this.content.version }); this.message = 'Content draft saved.'; await this.loadContent() } catch (error) { this.handleError(error, 'content', value, next => this.saveContent(next)) } },
    ...
    async saveHelp (draft) { const value = draft || { ...this.helpForm }; try { this.clearStatus(); this.help = await this.$C311.provider.updateAdminHelp(this.helpKey, { language: this.helpLanguage, body: value.body }, { expectedVersion: this.help.version }); this.helpForm = { body: this.help.body }; this.message = 'Help saved.'; await this.loadHelp() } catch (error) { this.handleError(error, 'help', value, next => this.saveHelp(next)) } },
    ...
    async saveCategory (category, draft) { const value = draft || { code: category.code, active: category.active, labels: { ...category.labels }, version: category.version }; try { this.clearStatus(); Object.assign(category, await this.$C311.provider.updateAdminCategory(category.code, { code: category.code, active: category.active, labels: category.labels }, { expectedVersion: category.version })); this.message = 'Category saved.' } catch (error) { this.handleError(error, 'category', value, next => this.saveCategory(category, next)) } },
