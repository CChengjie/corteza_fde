...
func assembleContract(document *ContractDocument) {
	addProvisions(document,
		"4.1.1", "4.3.3", "6.4.3", "6.5.4", "6.7.2", "6.7.3",
		...
		"10.5.1", "10.5.6", "10.7.1", "11.4.5", "11.5.3", "12.1.1", "12.2.1", "12.2.2", "12.2.3", "13.1.2", "15.4.1",
	)
	...
	document.Decisions = append(document.Decisions,
		...
		ContractDecision{
			...
			Decision:   "application_role contains the eight permission-matrix roles; oidc_actor_type and actor_role remain the narrower identity-provider wire vocabularies; audit_actor_type is independently constrained",
		},
		ContractDecision{
...
func contractProtocol() map[string]interface{} {
	return map[string]interface{}{
		...
		"optimistic_concurrency": map[string]interface{}{
			...
			"applies_to":     []string{"service_request", "workflow", "branding", "content", "custom_field"},
			"missing_status": 428,
		...
		"idempotency": map[string]interface{}{
			...
			"required_for":      []string{"portal_service_request_submit", "service_request_create", "mail_send", "workflow_action_execute", "civicworks_work_order_create"},
...
func applyEndpointDirections(document *ContractDocument) {
	for name, endpoint := range document.Endpoints {
		...
		if endpoint.Scope != "" {
			endpoint.Authentication = "OAuth 2.0 bearer token"
		}
		if name == "workflow_action_execute" {
			endpoint.Direction = EndpointConsumedByCRM
			endpoint.Authentication = "OAuth 2.0 client credentials"
